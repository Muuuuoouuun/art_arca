      ...
      <div className="bg-[#F2F0ED] min-h-screen text-[#1a1a1a]">
        <header className="px-8 md:px-24 pt-28 pb-16 border-b border-stone-200">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <p className="text-[9px] uppercase tracking-[0.4em] text-stone-400 mb-4">My Collection</p>
              <h1 className="text-5xl md:text-7xl font-serif tracking-tight">Bookmarks</h1>
            </div>
            <p className="text-sm text-stone-500">{saved.length}개의 저장된 전시</p>
          </div>
        </header>

        <section className="px-8 md:px-24 py-16 min-h-[50vh]">
          {saved.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 text-center">
              <div className="w-16 h-16 border border-stone-200 flex items-center justify-center mb-8">
                <svg width={24} height={24} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} fill="none" className="text-stone-300">
                  <path d="M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z" />
                </svg>
              </div>
              <p className="text-lg font-light text-stone-500 mb-2">저장한 전시가 없습니다</p>
              <p className="text-sm text-stone-400 mb-10">마음에 드는 전시에 북마크를 눌러 저장하세요.</p>
              <Link
                href="/exhibitions"
                className="text-[10px] uppercase tracking-[0.2em] border border-stone-300 text-stone-600 px-10 py-4 hover:border-stone-900 hover:text-stone-900 transition-all"
              >
                전시 둘러보기
              </Link>
            </div>
              ...
              {saved.map((ex, i) => (
                <Link key={ex.id} href={`/exhibitions/${ex.id}`} className="group block">
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <span className="text-[8px] uppercase tracking-[0.2em] font-bold text-stone-400">{ex.category}</span>
