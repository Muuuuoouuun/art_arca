...
export default function ExhibitionsGrid({ exhibitions }: Props) {
  ...
  return (
    <>
      ...
      <div className="px-8 md:px-24 pt-8 pb-0">
        <div className="relative max-w-sm">
          <input
            ...
            className="w-full bg-transparent border-b border-stone-300 text-sm text-stone-800 py-2.5 pr-8 focus:outline-none focus:border-stone-700 placeholder:text-stone-300 transition-colors"
          />
          {query ? (
            <button
              onClick={() => setQuery("")}
              className="absolute right-0 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-700 transition-colors"
              aria-label="검색어 지우기"
            >
            ...
            <svg className="absolute right-0 top-1/2 -translate-y-1/2 text-stone-300" width={15} height={15} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} fill="none">
      ...
      {/* 카테고리 필터 */}
      <div className="px-8 md:px-24 py-5 border-b border-stone-100 flex gap-6 overflow-x-auto">
        {categories.map((cat) => (
          <button
            key={cat}
      ...
      <section className="px-8 md:px-24 py-16">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <p className="text-lg font-light text-stone-400 mb-3">검색 결과가 없습니다</p>
            <button
              onClick={() => { setQuery(""); setActiveCategory("전체"); }}
              className="text-[9px] uppercase tracking-[0.2em] text-stone-400 hover:text-stone-700 underline underline-offset-4 transition-colors"
            >
              필터 초기화
            </button>
          </div>
